...
int vterm_screen_get_cell_colors(const VTermScreen *screen,
    VTermPos pos, VTermColor *fg, VTermColor *bg,
    uint32_t *ch, int *is_reverse)
{
  ScreenCell *intcell = getcell(screen, pos.row, pos.col);
  if(!intcell)
    return 0;
  ...
  *bg = intcell->pen.bg;
  *ch = intcell->chars[0];
  *is_reverse = intcell->pen.reverse ^ screen->global_reverse;
  return 1;
}
